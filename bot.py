...
ADMIN_IDS = list(map(int, os.getenv("ADMIN_IDS", "5804264903").split(",")))  # يمكن وضع أكثر من معرف مفصول بفاصلة
PAYMENT_NUMBER = os.getenv("PAYMENT_NUMBER")
PRICE = int(os.getenv("PRICE", 5000))
...
# إضافة الرقم الافتراضي إذا لم يكن موجودًا
c.execute("INSERT OR IGNORE INTO payment_methods (id, number) VALUES (1, ?)", (PAYMENT_NUMBER,))
conn.commit()
...
def admin_menu():
    ...
    markup.add(InlineKeyboardButton("✏️ تعديل أرقام الدفع", callback_data="edit_payment"))
    return markup
...
def handle_buttons(call):
    ...
    if call.data == "buy_ticket":
        ...
        c.execute("SELECT number FROM payment_methods WHERE id=1")
        pay_number = c.fetchone()[0]
    ...
    elif call.data == "edit_payment" and user_id in ADMIN_IDS:
        bot.send_message(user_id, "أرسل الرقم الجديد ليتم حفظه كرقم دفع رسمي:")
...
def handle_messages(message):
    ...
    try:
        if user_id in ADMIN_IDS:
            ...
            elif message.text.startswith("+") or message.text.isdigit():
                # تعديل رقم الدفع من قبل الأدمن
                new_number = message.text.strip()
                c.execute("UPDATE payment_methods SET number=? WHERE id=1", (new_number,))
                conn.commit()
                bot.reply_to(message, f"✅ تم تحديث رقم الدفع إلى: {new_number}")
            else:
                # بث رسالة جماعية
